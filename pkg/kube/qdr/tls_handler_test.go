...
func TestDisableTlsSupport(t *testing.T) {
	...
	for _, test := range tests {
		...
		fmt.Println(test.name)
		err := DisableTlsSupport(test.tlsCredentials, test.removeSslProfile, test.getSecret, test.deleteSecret)
		if test.expectedError != "" {
			assert.Error(t, err, test.expectedError, test.name)
		}
...
func (m MockAddSslProfileFunc) ok(sslProfile qdr.SslProfile) error {
	return nil
}
func (m MockAddSslProfileFunc) error(sslProfile qdr.SslProfile) error {
