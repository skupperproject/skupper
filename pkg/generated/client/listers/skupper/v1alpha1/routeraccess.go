...
type RouterAccessLister interface {
	...
	List(selector labels.Selector) (ret []*v1alpha1.RouterAccess, err error)
...
// List lists all RouterAccesses in the indexer.
func (s *routerAccessLister) List(selector labels.Selector) (ret []*v1alpha1.RouterAccess, err error) {
	err = cache.ListAll(s.indexer, selector, func(m interface{}) {
		ret = append(ret, m.(*v1alpha1.RouterAccess))
	})
	return ret, err
}
...
type RouterAccessNamespaceLister interface {
	...
	// Objects returned here must be treated as read-only.
	Get(name string) (*v1alpha1.RouterAccess, error)
...
// List lists all RouterAccesses in the indexer for a given namespace.
func (s routerAccessNamespaceLister) List(selector labels.Selector) (ret []*v1alpha1.RouterAccess, err error) {
...
// Get retrieves the RouterAccess from the indexer for a given namespace and name.
func (s routerAccessNamespaceLister) Get(name string) (*v1alpha1.RouterAccess, error) {
	obj, exists, err := s.indexer.GetByKey(s.namespace + "/" + name)
	if err != nil {
		return nil, err
	}
	if !exists {
		return nil, errors.NewNotFound(v1alpha1.Resource("routeraccess"), name)
	}
	return obj.(*v1alpha1.RouterAccess), nil
}
