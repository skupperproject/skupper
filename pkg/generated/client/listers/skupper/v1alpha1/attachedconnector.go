...
type AttachedConnectorLister interface {
	...
	List(selector labels.Selector) (ret []*v1alpha1.AttachedConnector, err error)
...
// List lists all AttachedConnectors in the indexer.
func (s *attachedConnectorLister) List(selector labels.Selector) (ret []*v1alpha1.AttachedConnector, err error) {
	err = cache.ListAll(s.indexer, selector, func(m interface{}) {
		ret = append(ret, m.(*v1alpha1.AttachedConnector))
	})
	return ret, err
}
...
type AttachedConnectorNamespaceLister interface {
	...
	// Objects returned here must be treated as read-only.
	Get(name string) (*v1alpha1.AttachedConnector, error)
...
// List lists all AttachedConnectors in the indexer for a given namespace.
func (s attachedConnectorNamespaceLister) List(selector labels.Selector) (ret []*v1alpha1.AttachedConnector, err error) {
...
// Get retrieves the AttachedConnector from the indexer for a given namespace and name.
func (s attachedConnectorNamespaceLister) Get(name string) (*v1alpha1.AttachedConnector, error) {
	obj, exists, err := s.indexer.GetByKey(s.namespace + "/" + name)
	if err != nil {
		return nil, err
	}
	if !exists {
		return nil, errors.NewNotFound(v1alpha1.Resource("attachedconnector"), name)
	}
	return obj.(*v1alpha1.AttachedConnector), nil
}
