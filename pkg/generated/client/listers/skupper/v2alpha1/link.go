...
// List lists all Links in the indexer.
func (s *linkLister) List(selector labels.Selector) (ret []*v2alpha1.Link, err error) {
	err = cache.ListAll(s.indexer, selector, func(m interface{}) {
		ret = append(ret, m.(*v2alpha1.Link))
	})
	return ret, err
}
...
// List lists all Links in the indexer for a given namespace.
func (s linkNamespaceLister) List(selector labels.Selector) (ret []*v2alpha1.Link, err error) {
...
// Get retrieves the Link from the indexer for a given namespace and name.
func (s linkNamespaceLister) Get(name string) (*v2alpha1.Link, error) {
	obj, exists, err := s.indexer.GetByKey(s.namespace + "/" + name)
	if err != nil {
		return nil, err
	}
	if !exists {
		return nil, errors.NewNotFound(v2alpha1.Resource("link"), name)
	}
	return obj.(*v2alpha1.Link), nil
}
