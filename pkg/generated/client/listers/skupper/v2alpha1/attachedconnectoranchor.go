...
type AttachedConnectorAnchorLister interface {
	...
	List(selector labels.Selector) (ret []*v2alpha1.AttachedConnectorAnchor, err error)
...
// List lists all AttachedConnectorAnchors in the indexer.
func (s *attachedConnectorAnchorLister) List(selector labels.Selector) (ret []*v2alpha1.AttachedConnectorAnchor, err error) {
	err = cache.ListAll(s.indexer, selector, func(m interface{}) {
		ret = append(ret, m.(*v2alpha1.AttachedConnectorAnchor))
	})
	return ret, err
}
...
type AttachedConnectorAnchorNamespaceLister interface {
	...
	// Objects returned here must be treated as read-only.
	Get(name string) (*v2alpha1.AttachedConnectorAnchor, error)
...
// List lists all AttachedConnectorAnchors in the indexer for a given namespace.
func (s attachedConnectorAnchorNamespaceLister) List(selector labels.Selector) (ret []*v2alpha1.AttachedConnectorAnchor, err error) {
...
// Get retrieves the AttachedConnectorAnchor from the indexer for a given namespace and name.
func (s attachedConnectorAnchorNamespaceLister) Get(name string) (*v2alpha1.AttachedConnectorAnchor, error) {
	obj, exists, err := s.indexer.GetByKey(s.namespace + "/" + name)
	if err != nil {
		return nil, err
	}
	if !exists {
		return nil, errors.NewNotFound(v2alpha1.Resource("attachedconnectoranchor"), name)
	}
	return obj.(*v2alpha1.AttachedConnectorAnchor), nil
}
