...
func ConnectorsDifference(actual map[string]Connector, desired *RouterConfig, ignorePrefix *string) *ConnectorDifference {
	...
	for key, v1 := range desired.Connectors {
		...
		if ok && v1.IsLinkConnector() && !v1.Equivalent(actual[key]) {
			result.Added = append(result.Added, v1)
		}
	}
	for key, v1 := range actual {
		...
		//if the link connector exists but has changed some of its values, the connector's former version has to be deleted
		if ok && v1.IsLinkConnector() && !v1.Equivalent(desired.Connectors[key]) && allowedToDelete {
			result.Deleted = append(result.Deleted, v1)
		}
	}
	return &result
