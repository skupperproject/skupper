...
func main() {
	...
	snap, err := sweeper.Gather(*skmanageBin, *url)
	if err != nil {
	...
	fmt.Printf("%-4s %-25s %-10s %-10s\n", "DIR", "HOST", "UPTIME(s)", "LASTDLV(s)")
	for _, c := range snap.TCPConns {
		fmt.Printf("%-4s %-25s %-10s %-10s\n",
			c.Dir, c.Host, ptrStr(c.UptimeSeconds), ptrStr(c.LastDlvSeconds))
	}

	fmt.Println("\nsockets (host:port -> lastrcv/lastsnd s):")
