...
func (r *EdgeConnectivityTestRunner) Setup(ctx context.Context, testCase *TestCase, t *testing.T) {
	...
	for i := 0; i < int(createOptsPublic.Replicas); i++ {
		...
		// Create the router.
		err = pub1Cluster.VanClient.RouterCreate(ctx, *siteConfig, types.DefaultTimeout)
		assert.Assert(t, err)
	...
	assert.Assert(t, err)
	err = privateCluster.VanClient.RouterCreate(ctx, *siteConfig, types.DefaultTimeout)
	assert.Assert(t, err)
