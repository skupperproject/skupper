...
func (cli *VanClient) newGateway(ctx context.Context, gatewayName string, gatewayType string, configFile string, owner *metav1.OwnerReference) (string, error) {
	...
	_, err = cli.SecretManager(cli.GetNamespace()).CreateSecret(secret)
	if err != nil {
		...
		for _, service := range svcList {
			err = retry.RetryOnConflict(retry.DefaultRetry, func() error {
				...
				if si != nil && len(si.Targets) == 0 && si.Origin == "" {
					...
					_, err = cli.ServiceManager(cli.GetNamespace()).UpdateService(&service)
					return err
				}
			})
		}
	...
	if si == nil {
		...
		err = retry.RetryOnConflict(retry.DefaultRetry, func() error {
			...
			if err == nil {
				...
				_, err = cli.ServiceManager(cli.GetNamespace()).UpdateService(svc)
				return err
			} else {
				return err
			}
