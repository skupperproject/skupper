...
func TestEdgeConnectivity(t *testing.T) {
	...
	for testIndex, test := range tests {
		...
		if verbose >= 1 {
			fp(os.Stdout, "\n\n%stest %d: %s%s%s\n", yellow, testIndex+1, cyan, test.name, resetColor)
		}
		...
		for _, namespace := range test.namespaces {
			...
			// We need to call SiteConfigCreate() with this structure,
			// and then use its return value as an argument to RouterCreate()
			// It is possible to call RouterCreate() with a handmade siteConfig
			// structure, but that doesn't work for a real-cluster test, because it
			// misses the side-effect in SiteConfigCreate() of actually creating the site.
