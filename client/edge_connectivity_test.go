...
func getConnectivity(
	...
	for {
		...
		if response != nil {
			info(testName, fmt.Sprintf("%d : (%d,%d) != (%d,%d)", timeout, response.Status.ConnectedSites.Direct, response.Status.ConnectedSites.Indirect, expectedDirect, expectedIndirect))
		}
