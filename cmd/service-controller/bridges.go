...
func mountServiceCertificateByName(address string, name string, c *Controller) error {

	_, err := c.vanClient.KubeClient.CoreV1().Secrets(c.vanClient.Namespace).Get(name, metav1.GetOptions{})

	// If the requested certificate is one generated by skupper it can be generated in other sites as well
	if err != nil && strings.HasPrefix(name, types.SkupperServiceCertPrefix) {
		serviceSecret, err := kube.CreateSecretsForService(address, c.vanClient.Namespace, address, name, c.vanClient.KubeClient)
		...
		log.Printf("Error when trying to mount service certificate: %v\n", err.Error())
		return err
	}
