...
func (c *Controller) updateServiceBindings(required types.ServiceInterface, portAllocations map[string][]int) error {
	...
	if bindings == nil {
		...
		if len(required.TlsCredentials) > 0 {
			...
			err := handleTlsSupport(sb.address, required.TlsCredentials, c)
			if err != nil {
				return err
			}
		}
		...
		if bindings.tlsCredentials != required.TlsCredentials {
			...
			err := handleTlsSupport(required.Address, bindings.tlsCredentials, c)
			if err != nil {
				return err
			}
...
func handleTlsSupport(address string, credentialsName string, c *Controller) error {

	_, err := c.vanClient.KubeClient.CoreV1().Secrets(c.vanClient.Namespace).Get(credentialsName, metav1.GetOptions{})

	// If the requested certificate is one generated by skupper it can be generated in other sites as well
	if err != nil && strings.HasPrefix(credentialsName, types.SkupperServiceCertPrefix) {
		serviceSecret, err := c.vanClient.CreateSecretForService(address, address, credentialsName)
		...
		log.Printf("Error when trying to handle TLS support: %v\n", err.Error())
		return err
	}
