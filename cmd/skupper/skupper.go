...
installation that can then be connected to other skupper installations`,
		...
		RunE: func(cmd *cobra.Command, args []string) error {
			...
			if routerCreateOpts.Router.DebugMode != "" {
				if routerCreateOpts.Router.DebugMode != "asan" && routerCreateOpts.Router.DebugMode != "tsan" && routerCreateOpts.Router.DebugMode != "gdb" {
					return fmt.Errorf("Bad value for --router-debug-mode: %s (use 'asan', 'tsan' or 'gdb')", routerCreateOpts.Router.DebugMode)
				}
			}
	...
	cmd.Flags().StringVarP(&routerCreateOpts.Router.DebugMode, "router-debug-mode", "", "", "Enable debug mode for router ('asan', 'tsan' or 'gdb' are valid values)")
